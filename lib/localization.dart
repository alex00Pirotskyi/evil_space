import 'package:flutter/widgets.dart';

enum AppLanguage {
  en('en'),
  ru('ru'),
  vi('vi');

  const AppLanguage(this.code);

  final String code;

  static AppLanguage fromLocale(Locale locale) {
    final languageCode = locale.languageCode.toLowerCase();
    return AppLanguage.values.firstWhere(
      (language) => language.code == languageCode,
      orElse: () => AppLanguage.en,
    );
  }
}

class LocalizationController extends ChangeNotifier {
  LocalizationController([this._language = AppLanguage.en]);

  factory LocalizationController.fromPlatform() {
    final locale = WidgetsBinding.instance.platformDispatcher.locale;
    return LocalizationController(AppLanguage.fromLocale(locale));
  }

  AppLanguage _language;

  AppLanguage get language => _language;

  void setLanguage(AppLanguage language) {
    if (_language == language) return;
    _language = language;
    notifyListeners();
  }

  String t(String key) {
    return strings[_language.code]?[key] ?? strings['en']?[key] ?? key;
  }

  static const Map<String, Map<String, String>> strings = {
    'en': {
      'brand_daily': 'EVIL SPACE / DAILY',
      'hero_heading': 'Evil Space coworking space in Nha Trang',
      'hero_kicker': 'NHA TRANG / COWORKING / OPEN DAILY 11:00–23:00',
      'desk_free': 'DESK AVAILABLE',
      'desks_free': 'DESKS AVAILABLE',
      'occupied': 'OCCUPIED',
      'updated_today': 'UPDATED TODAY',
      'updated': 'UPDATED',
      'local_data': 'LOCAL STATUS',
      'availability_kicker': 'TODAY AT EVIL SPACE',
      'availability_action': 'GET A DESK TODAY',
      'booking_today': 'TODAY',
      'booking_tomorrow': 'TOMORROW',
      'booking_title_today': 'GET A DESK TODAY',
      'booking_title_tomorrow': 'GET A DESK TOMORROW',
      'free_short': 'AVAILABLE',
      'booking_title': 'GET A DESK TODAY',
      'booking_name': 'NAME',
      'booking_phone': 'PHONE',
      'booking_telegram': 'TELEGRAM',
      'booking_contact': 'CONTACT',
      'booking_send': 'SEND REQUEST',
      'booking_tg_button': 'TG · BOOK WITH TELEGRAM',
      'booking_tg_copy':
          'ONE TAP. TELEGRAM WILL IDENTIFY YOU — NO NAME OR PHONE NEEDED.',
      'booking_or': 'OR',
      'booking_phone_copy': 'NO TELEGRAM? LEAVE YOUR NAME AND PHONE.',
      'booking_sent': 'REQUEST SENT TO EVIL SPACE',
      'booking_pending': 'YOUR REQUEST IS PENDING',
      'booking_accepted': 'YOU HAVE A BOOKING TODAY',
      'booking_declined': 'YOUR REQUEST WAS DECLINED',
      'booking_cancelled': 'YOUR BOOKING WAS CANCELLED',
      'booking_delete': 'DELETE REQUEST',
      'booking_deleted': 'REQUEST DELETED',
      'booking_again': 'MAKE ANOTHER REQUEST',
      'booking_connect_telegram': 'CONNECT TELEGRAM UPDATES',
      'booking_telegram_connected': 'TELEGRAM UPDATES CONNECTED',
      'booking_connect_title': 'GET UPDATES IN TELEGRAM?',
      'booking_connect_copy': 'Connect once and the Evil Space bot will message you when the booking is accepted or declined. After acceptance you can cancel the booking and get Wi-Fi access from Telegram.',
      'booking_save_title': 'SAVE FOR NEXT TIME?',
      'booking_save_copy': 'Save this contact only on this device for one-click booking next time.',
      'booking_save': 'SAVE',
      'booking_not_now': 'NOT NOW',
      'booking_saved': 'ONE-CLICK CONTACT SAVED',
      'booking_forget': 'FORGET',
      'message_zalo': 'MESSAGE ZALO',
      'prices_title': 'SIMPLE PRICES',
      'about_title': 'THE SPACE',
      'amenity_chairs': 'KOREAN ERGO',
      'amenity_wifi': '800 MBPS',
      'amenity_desks': '140 × 70 CM',
      'amenity_air': 'A/C',
      'amenity_coffee': 'COFFEE BAR',
      'amenity_panels': 'ACOUSTIC',
      'amenity_chairs_detail': 'Korean ergonomic chairs',
      'amenity_wifi_detail': '800 Mbps fiber Wi-Fi',
      'amenity_desks_detail': '140 by 70 centimeter desks',
      'amenity_air_detail': 'Air conditioning',
      'amenity_coffee_detail': 'Cafeteria with coffee',
      'amenity_panels_detail': 'Acoustic panels between desks',
      'promo_label': 'PROMO',
      'price_day_pass': 'DAY PASS',
      'price_month': 'ONE MONTH',
      'price_locker': 'PERSONAL LOCKER',
      'opening_title': 'OPENING 20 OCTOBER',
      'opening_studio': 'PODCAST / STUDIO ROOM',
      'opening_lecture': 'LECTURE ROOM',
      'coming_soon': 'COMING SOON',
      'now_open': 'NOW OPEN',
      'notes_title': "TODAY'S NOTE",
      'visit_title': 'FIND EVIL SPACE',
      'contact_location': '60 CAO VĂN BÉ / BẮC NHA TRANG / KHÁNH HÒA',
      'contact_instagram': 'INSTAGRAM',
      'contact_map': 'PHOTOS & REVIEWS',
      'contact_directions': 'DIRECTIONS',
      'contact_zalo': 'MESSAGE ZALO',
      'contact_telegram': 'TELEGRAM',
      'contact_phone': 'CALL',
      'page_one': 'PAGE 1 OF 1',
    },
    'ru': {
      'brand_daily': 'EVIL SPACE / DAILY',
      'hero_heading': 'Evil Space — коворкинг в Нячанге',
      'hero_kicker': 'НЯЧАНГ / КОВОРКИНГ / ЕЖЕДНЕВНО 11:00–23:00',
      'desk_free': 'СВОБОДНЫЙ СТОЛ',
      'desks_free': 'СВОБОДНЫХ СТОЛОВ',
      'occupied': 'ЗАНЯТО',
      'updated_today': 'ОБНОВЛЕНО СЕГОДНЯ',
      'updated': 'ОБНОВЛЕНО',
      'local_data': 'ЛОКАЛЬНЫЙ СТАТУС',
      'availability_kicker': 'СЕГОДНЯ В EVIL SPACE',
      'availability_action': 'ЗАБРОНИРОВАТЬ СТОЛ',
      'booking_today': 'СЕГОДНЯ',
      'booking_tomorrow': 'ЗАВТРА',
      'booking_title_today': 'ЗАБРОНИРОВАТЬ СТОЛ НА СЕГОДНЯ',
      'booking_title_tomorrow': 'ЗАБРОНИРОВАТЬ СТОЛ НА ЗАВТРА',
      'free_short': 'СВОБ.',
      'booking_title': 'ЗАБРОНИРОВАТЬ СТОЛ',
      'booking_name': 'ИМЯ',
      'booking_phone': 'ТЕЛЕФОН',
      'booking_telegram': 'TELEGRAM',
      'booking_contact': 'КОНТАКТ',
      'booking_send': 'ОТПРАВИТЬ',
      'booking_tg_button': 'TG · ЗАБРОНИРОВАТЬ ЧЕРЕЗ TELEGRAM',
      'booking_tg_copy':
          'ОДИН КЛИК. TELEGRAM ОПРЕДЕЛИТ ВАС — ИМЯ И ТЕЛЕФОН НЕ НУЖНЫ.',
      'booking_or': 'ИЛИ',
      'booking_phone_copy': 'НЕТ TELEGRAM? ОСТАВЬТЕ ИМЯ И ТЕЛЕФОН.',
      'booking_sent': 'ЗАПРОС ОТПРАВЛЕН В EVIL SPACE',
      'booking_pending': 'ВАШ ЗАПРОС ОЖИДАЕТ ПОДТВЕРЖДЕНИЯ',
      'booking_accepted': 'У ВАС ЕСТЬ БРОНЬ НА СЕГОДНЯ',
      'booking_declined': 'ВАШ ЗАПРОС ОТКЛОНЁН',
      'booking_cancelled': 'ВАША БРОНЬ ОТМЕНЕНА',
      'booking_delete': 'УДАЛИТЬ ЗАПРОС',
      'booking_deleted': 'ЗАПРОС УДАЛЁН',
      'booking_again': 'СОЗДАТЬ НОВЫЙ ЗАПРОС',
      'booking_connect_telegram': 'ПОДКЛЮЧИТЬ TELEGRAM',
      'booking_telegram_connected': 'ОБНОВЛЕНИЯ TELEGRAM ПОДКЛЮЧЕНЫ',
      'booking_connect_title': 'ПОЛУЧАТЬ ОБНОВЛЕНИЯ В TELEGRAM?',
      'booking_connect_copy': 'Подключитесь один раз, и бот Evil Space напишет, когда бронь будет принята или отклонена. После подтверждения через Telegram можно отменить бронь и получить Wi-Fi.',
      'booking_save_title': 'СОХРАНИТЬ НА БУДУЩЕЕ?',
      'booking_save_copy': 'Сохранить контакт только на этом устройстве для бронирования в один клик в следующий раз.',
      'booking_save': 'СОХРАНИТЬ',
      'booking_not_now': 'НЕ СЕЙЧАС',
      'booking_saved': 'КОНТАКТ СОХРАНЁН ДЛЯ 1 КЛИКА',
      'booking_forget': 'ЗАБЫТЬ',
      'message_zalo': 'НАПИСАТЬ В ZALO',
      'prices_title': 'ПРОСТЫЕ ЦЕНЫ',
      'about_title': 'ПРОСТРАНСТВО',
      'amenity_chairs': 'ЭРГОКРЕСЛА',
      'amenity_wifi': '800 МБИТ/С',
      'amenity_desks': '140 × 70 СМ',
      'amenity_air': 'КОНДИЦИОНЕР',
      'amenity_coffee': 'КОФЕ-БАР',
      'amenity_panels': 'АКУСТИКА',
      'amenity_chairs_detail': 'Корейские эргономичные кресла',
      'amenity_wifi_detail': 'Оптоволоконный Wi-Fi 800 Мбит/с',
      'amenity_desks_detail': 'Рабочие столы 140 на 70 сантиметров',
      'amenity_air_detail': 'Кондиционер',
      'amenity_coffee_detail': 'Кафетерий с кофе',
      'amenity_panels_detail': 'Акустические панели между столами',
      'promo_label': 'АКЦИЯ',
      'price_day_pass': 'ДЕНЬ',
      'price_month': 'ОДИН МЕСЯЦ',
      'price_locker': 'ЛИЧНЫЙ ШКАФЧИК',
      'opening_title': 'ОТКРЫТИЕ 20 ОКТЯБРЯ',
      'opening_studio': 'ПОДКАСТ / СТУДИЯ',
      'opening_lecture': 'ЛЕКЦИОННЫЙ ЗАЛ',
      'coming_soon': 'СКОРО',
      'now_open': 'УЖЕ ОТКРЫТО',
      'notes_title': 'ЗАМЕТКА НА СЕГОДНЯ',
      'visit_title': 'НАЙТИ EVIL SPACE',
      'contact_location': '60 CAO VĂN BÉ / СЕВЕРНЫЙ НЯЧАНГ / KHÁNH HÒA',
      'contact_instagram': 'INSTAGRAM',
      'contact_map': 'ФОТО И ОТЗЫВЫ',
      'contact_directions': 'МАРШРУТ',
      'contact_zalo': 'НАПИСАТЬ В ZALO',
      'contact_telegram': 'TELEGRAM',
      'contact_phone': 'ПОЗВОНИТЬ',
      'page_one': 'СТРАНИЦА 1 ИЗ 1',
    },
    'vi': {
      'brand_daily': 'EVIL SPACE / DAILY',
      'hero_heading': 'Evil Space — không gian làm việc chung tại Nha Trang',
      'hero_kicker': 'NHA TRANG / COWORKING / MỞ CỬA HẰNG NGÀY 11:00–23:00',
      'desk_free': 'BÀN TRỐNG',
      'desks_free': 'BÀN TRỐNG',
      'occupied': 'ĐANG DÙNG',
      'updated_today': 'CẬP NHẬT HÔM NAY',
      'updated': 'CẬP NHẬT',
      'local_data': 'TRẠNG THÁI NỘI BỘ',
      'availability_kicker': 'HÔM NAY TẠI EVIL SPACE',
      'availability_action': 'ĐẶT BÀN HÔM NAY',
      'booking_today': 'HÔM NAY',
      'booking_tomorrow': 'NGÀY MAI',
      'booking_title_today': 'ĐẶT BÀN HÔM NAY',
      'booking_title_tomorrow': 'ĐẶT BÀN NGÀY MAI',
      'free_short': 'TRỐNG',
      'booking_title': 'ĐẶT BÀN HÔM NAY',
      'booking_name': 'TÊN',
      'booking_phone': 'ĐIỆN THOẠI',
      'booking_telegram': 'TELEGRAM',
      'booking_contact': 'LIÊN HỆ',
      'booking_send': 'GỬI YÊU CẦU',
      'booking_tg_button': 'TG · ĐẶT QUA TELEGRAM',
      'booking_tg_copy': 'MỘT CHẠM. TELEGRAM SẼ NHẬN DIỆN BẠN — KHÔNG CẦN TÊN HAY SỐ ĐIỆN THOẠI.',
      'booking_or': 'HOẶC',
      'booking_phone_copy': 'KHÔNG DÙNG TELEGRAM? ĐỂ LẠI TÊN VÀ SỐ ĐIỆN THOẠI.',
      'booking_sent': 'ĐÃ GỬI YÊU CẦU ĐẾN EVIL SPACE',
      'booking_pending': 'YÊU CẦU CỦA BẠN ĐANG CHỜ XÁC NHẬN',
      'booking_accepted': 'BẠN ĐÃ CÓ BÀN HÔM NAY',
      'booking_declined': 'YÊU CẦU CỦA BẠN ĐÃ BỊ TỪ CHỐI',
      'booking_cancelled': 'ĐẶT BÀN CỦA BẠN ĐÃ BỊ HỦY',
      'booking_delete': 'XOÁ YÊU CẦU',
      'booking_deleted': 'ĐÃ XOÁ YÊU CẦU',
      'booking_again': 'GỬI YÊU CẦU MỚI',
      'booking_connect_telegram': 'KẾT NỐI TELEGRAM',
      'booking_telegram_connected': 'ĐÃ KẾT NỐI CẬP NHẬT TELEGRAM',
      'booking_connect_title': 'NHẬN CẬP NHẬT QUA TELEGRAM?',
      'booking_connect_copy': 'Kết nối một lần để bot Evil Space báo khi yêu cầu được chấp nhận hoặc từ chối. Sau khi được chấp nhận, bạn có thể hủy đặt bàn và lấy Wi-Fi ngay trong Telegram.',
      'booking_save_title': 'LƯU CHO LẦN SAU?',
      'booking_save_copy':
          'Chỉ lưu liên hệ trên thiết bị này để đặt bàn một chạm lần sau.',
      'booking_save': 'LƯU',
      'booking_not_now': 'KHÔNG LÚC NÀY',
      'booking_saved': 'ĐÃ LƯU LIÊN HỆ MỘT CHẠM',
      'booking_forget': 'XOÁ',
      'message_zalo': 'NHẮN ZALO',
      'prices_title': 'BẢNG GIÁ',
      'about_title': 'KHÔNG GIAN',
      'amenity_chairs': 'GHẾ ERGO HÀN',
      'amenity_wifi': '800 MBPS',
      'amenity_desks': '140 × 70 CM',
      'amenity_air': 'MÁY LẠNH',
      'amenity_coffee': 'QUẦY CÀ PHÊ',
      'amenity_panels': 'VÁCH TIÊU ÂM',
      'amenity_chairs_detail': 'Ghế công thái học Hàn Quốc',
      'amenity_wifi_detail': 'Wi-Fi cáp quang 800 Mbps',
      'amenity_desks_detail': 'Bàn làm việc 140 × 70 cm',
      'amenity_air_detail': 'Máy lạnh',
      'amenity_coffee_detail': 'Khu cà phê phục vụ cà phê',
      'amenity_panels_detail': 'Vách tiêu âm giữa các bàn',
      'promo_label': 'KHUYẾN MÃI',
      'price_day_pass': 'VÉ NGÀY',
      'price_month': 'MỘT THÁNG',
      'price_locker': 'TỦ CÁ NHÂN',
      'opening_title': 'KHAI TRƯƠNG 20 THÁNG 10',
      'opening_studio': 'PHÒNG PODCAST / STUDIO',
      'opening_lecture': 'PHÒNG HỘI THẢO',
      'coming_soon': 'SẮP RA MẮT',
      'now_open': 'ĐÃ MỞ CỬA',
      'notes_title': 'GHI CHÚ HÔM NAY',
      'visit_title': 'TÌM EVIL SPACE',
      'contact_location': '60 CAO VĂN BÉ / BẮC NHA TRANG / KHÁNH HÒA',
      'contact_instagram': 'INSTAGRAM',
      'contact_map': 'ẢNH & ĐÁNH GIÁ',
      'contact_directions': 'CHỈ ĐƯỜNG',
      'contact_zalo': 'NHẮN ZALO',
      'contact_telegram': 'TELEGRAM',
      'contact_phone': 'GỌI',
      'page_one': 'TRANG 1 / 1',
    },
  };
}
